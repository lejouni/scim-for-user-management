...
def lambda_handler(event, context):
    ...
    #Check is the given token valid BD Access Token or not.
    if event['authorizationToken']:
        token = event['authorizationToken'].split('Bearer')[-1].strip()
        hub = HubInstance(BD_URL, api_token=token, write_config_flag=False) 
        if hub.csrf_token:
